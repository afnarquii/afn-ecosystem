...
Si llega caption («este es parte del pago» / «Valida» / «Abono») con media, el runtime **bloquea el LLM de catálogo** (sin tools `data_find`/`local_index` — ni prefetch), espera el archivo, OCR + Gmail. **PROHIBIDO** preguntar «¿cuánto abonaste?», buscar la imagen en el workspace, decir «no me llegó la foto», o improvisar «estoy validando» sin pasar por `payment_proof`. Si el status de pago se perdió pero hay `orderCodigo` + foto de comprobante, el runtime reabre validación (soft awaiting) sin hardcode de vertical.
